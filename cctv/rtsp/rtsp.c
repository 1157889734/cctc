				...
				if((pcData[0] & 0xFF) == '$')          
				{
					...
					if ((cChIdentifier % 2) == 0)
					{
						...
						if (0 == iSpsFlag)
						{
							...
							{
                                RTSP_DEBUG(DEBUG_RTSP_LEVEL, "Send h264 sps frame, frame len %d\n", ptRtspConn->iSpsPpsLen);
...
int RtspConnect(PT_RTSP_CONN ptRtspConn, const char *pcUri)
{
    ...
    connectTimeout.tv_usec = 150*1000;
#endif
    if(setsockopt(ptRtspConn->iRtspSocket, SOL_SOCKET, SO_SNDTIMEO, (char *)&connectTimeout, sizeof(connectTimeout))==-1)
    {
    ...
    if ( setsockopt(ptRtspConn->iRtspSocket, SOL_SOCKET, SO_RCVBUF, (char *)&windowSize, \
                                windowSizeLen) < 0 )
    {
